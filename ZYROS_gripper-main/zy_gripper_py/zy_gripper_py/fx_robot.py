...
class Marvin_Robot:
    ...
    def set_cyr_state_on(self, state: int):
        '''摇操作模式开启
        :param state: 状态值
        :return:
            int: 1: True; 0: False
        ...
        try:
            state_int = ctypes.c_int(state)
            # 注意：该函数未以 extern "C" 导出，需使用 C++ 修饰后的符号名
            _cyr_on = getattr(self.robot, '_Z20OnSetTargetState_CYRi')
            return _cyr_on(state_int)
        except Exception as e:
            print(f'ERROR:{e}')

    def set_cyr_kd_params(self, K: list, D: list):
        '''摇操作阻抗参数设置
        :param K: list(7,1). 刚度参数
        :param D: list(7,1). 阻尼参数
        :return:
            int: 1: True; 0: False
        '''
        try:
            ...
            d_value = d_double(d0, d1, d2, d3, d4, d5, d6)
            # 注意：该函数未以 extern "C" 导出，需使用 C++ 修饰后的符号名
            _cyr_kd = getattr(self.robot, '_Z12OnSetCYRKD_APdS_')
            return _cyr_kd(k_value, d_value)
        except Exception as e:
            print(f'ERROR:{e}')

    def set_cyr_state_off(self, state: int):
        '''摇操作模式关闭
        :param state: 状态值
        :return:
            int: 1: True; 0: False
        ...
        try:
            state_int = ctypes.c_int(state)
            # 注意：该函数未以 extern "C" 导出，需使用 C++ 修饰后的符号名
            _cyr_off = getattr(self.robot, '_Z21OnSetTargetState_CYR1i')
            return _cyr_off(state_int)
        except Exception as e:
            print(f'ERROR:{e}')
