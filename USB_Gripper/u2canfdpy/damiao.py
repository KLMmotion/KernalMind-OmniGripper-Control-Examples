...
class Motor_Control:
    ...
    def write_motor_param(self, DM_Motor, RID: int, data):
        self.read_write_save.set()
...
if __name__ == "__main__":
    ...
    try:
        ...
        # cmd_pub = node.create_publisher(Float64, 'motor_cmd_position', 10)
        # feedback_pub = node.create_publisher(Float64, 'motor_feedback_position', 10)

        with Motor_Control(1000000, 5000000, "4BB11E89F87B84FC6A71766BADB28A1F", init_data1) as control:
            ...
            while running.is_set():
                ...
                pos_target = amplitude * (1 + math.sin(2 * math.pi * frequency * elapsed_time)) * 0.5
                vel_target = amplitude * math.pi * frequency * math.cos(2 * math.pi * frequency * elapsed_time)

                print(pos_target)

                control.control_mit(
                    control.getMotor(canid1),
                    kp=5.0,
                    kd=0.2,
                    q=pos_target,
                    dq=0,
                ...
                for id in range(1):
                    ...
                    print(
                        f"canid is: {canid1} pos: {pos} vel: {vel} effort: {tau} "
                        f"target: {pos_target:.4f} time(s): {interval}",
                        file=sys.stderr
                    )

                    # ROS2 发布部分，已注释
                    # cmd_msg = Float64()
                    # cmd_msg.data = pos_target
                    # cmd_pub.publish(cmd_msg)

                    # feedback_msg = Float64()
                    # feedback_msg.data = pos
