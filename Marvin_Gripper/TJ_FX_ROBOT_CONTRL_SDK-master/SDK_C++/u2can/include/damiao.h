...
#pragma pack()
    ...
    public:
         ...
         * @param is_left 是否为左臂电机，达妙手册中部分电机参数需要区分左右臂电机，默认为右臂电机
         *
         */
        Motor(DM_Motor_Type Motor_Type, Motor_id Slave_id, Motor_id Master_id,bool is_left = false)
                : Master_id(Master_id), Slave_id(Slave_id), Motor_Type(Motor_Type), is_left(is_left) {
        ...
        bool is_left_arm() const { return this->is_left; }
 ...
 */
    ...
    public:
        ...
        */
        Motor_Control()
        {
            printf("Motor_Control Constructed\n");
            if (use_arm_b_)
            {
                OnClearChDataB();
            }
        ...
        */
        void enable(const Motor& motor)
        {
            ...
            usleep(100000);//100ms
            long channel = motor.is_left_arm() ? 1 : 2;
        ...
        void refresh_motor_status(const Motor& motor)
        {
            ...
            serial_write(send_data, channel);
            this->receive(channel);
        }
        /*
        ...
        */
         ...
         */
        void receive(long read_channel)
        {
            serial_read(reinterpret_cast<uint8_t*>(&receive_data), sizeof(CAN_Receive_Frame), read_channel);
            ...
            if(receive_data.CMD == 0x11 && receive_data.frameEnd == 0x55)
            {
                ...
                uint16_t tau_uint = (uint16_t(data[4] & 0x0f) << 8) | data[5];
        ...
        void receive_param(long read_channel)
        {
            serial_read(reinterpret_cast<uint8_t*>(&receive_data), sizeof(CAN_Receive_Frame), read_channel);
        ...
        float read_motor_param(Motor &DM_Motor,uint8_t RID)
        {
            ...
            for(uint8_t i =0;i<max_retries;i++)
            {
                ...
                long channel = DM_Motor.is_left_arm() ? 1 : 2;
                receive_param(channel);
    ...
    private:
        // Wrapper methods for MarvinSDK communication
        bool serial_write(const can_send_frame data_frame, long channel = 1)
        {
            // Marvin 末端通道格式: [CAN_ID(4 bytes, little-endian)] + [DATA(8 bytes)]
            unsigned char data_ptr[256] = {0};
            data_ptr[0] = static_cast<unsigned char>(data_frame.canId & 0xFF);
            data_ptr[1] = static_cast<unsigned char>((data_frame.canId >> 8) & 0xFF);
            data_ptr[2] = static_cast<unsigned char>((data_frame.canId >> 16) & 0xFF);
            data_ptr[3] = static_cast<unsigned char>((data_frame.canId >> 24) & 0xFF);
            memcpy(data_ptr + 4, data_frame.data, 8);

           
            if (channel==2)
            {
                return OnSetChDataB(data_ptr, 12, 1);
            }
            else
            {
                return OnSetChDataA(data_ptr, 12, 1);
            }
        }

        long serial_read(uint8_t* data, size_t max_len, long read_channel, long* ret_ch = nullptr)
        {
            unsigned char data_ptr[256] = {0};
            long channel = 0;
            long result = 0;
            ...
            if (read_channel == 2)
            {
                result = OnGetChDataB(data_ptr, &channel);
            }
            else
            {
                result = OnGetChDataA(data_ptr, &channel);
            }
            ...
            // Marvin 末端通道返回格式: [CAN_ID(4 bytes)] + [DATA(8 bytes)]
            if (result >= 12 && max_len >= sizeof(CAN_Receive_Frame))
            {
                auto* frame = reinterpret_cast<CAN_Receive_Frame*>(data);
                memset(frame, 0, sizeof(CAN_Receive_Frame));
                frame->CMD = 0x11;
                frame->canDataLen = 8;
                frame->canIde = 0;
                frame->canRtr = 0;
                frame->canId = static_cast<uint32_t>(data_ptr[0]) |
                            (static_cast<uint32_t>(data_ptr[1]) << 8) |
                            (static_cast<uint32_t>(data_ptr[2]) << 16) |
                            (static_cast<uint32_t>(data_ptr[3]) << 24);
                memcpy(frame->canData, data_ptr + 4, 8);
                frame->frameEnd = 0x55;
                return sizeof(CAN_Receive_Frame);
            }

            if (result > 0 && result <= static_cast<long>(max_len))
            {
                memcpy(data, data_ptr, result);
            }
            return result;
        }
