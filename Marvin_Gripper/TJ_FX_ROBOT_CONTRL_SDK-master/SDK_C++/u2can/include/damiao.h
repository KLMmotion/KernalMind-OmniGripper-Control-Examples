 ...
 */
    ...
    public:
        ...
        * 默认串口为/dev/ttyACM0
        */
        Motor_Control(SerialPort::SharedPtr serial = nullptr)
        {
        ...
        */
        ...
        void refresh_motor_status(const Motor& motor)
        {
            ...
            serial_write(send_data, sizeof(can_send_frame));
            this->receive();
        }
        /*
    ...
    private:
        ...
        bool serial_write(const can_send_frame data_frame, long channel = 1)
        {
            ...
            // uint8_t *data = (uint8_t*)&data_frame.data;
            // uint len = 10;
            // if (len > 254) len = 254; // Reserve space for 2 inserted bytes
            
            // // Copy first 2 bytes
            // if (len >= 2) {
            //     data_ptr[0] = 0x01;
            //     data_ptr[1] = 0x00;
            //     // Insert two 0x00 bytes
            //     data_ptr[2] = 0x00;
            //     data_ptr[3] = 0x00;
            //     // Copy remaining bytes
            //     if (len > 2) {
            //         memcpy(data_ptr + 4, data, len - 2);
            //     }

            //     std::vector<uint8_t> bytes(data_ptr, data_ptr + len + 2);
            ...
            //     }
            //     printf("\n");
            //     return  OnSetChDataA(bytes.data(), static_cast<long>(bytes.size()), 1);
            // }
        ...
        long serial_read(uint8_t* data, size_t max_len, long* ret_ch = nullptr)
        {
            unsigned char data_ptr[256];
            long channel = 0;
            long result = OnGetChDataA(data_ptr, &channel);
